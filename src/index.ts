...
console.log('dates:', dates['1838-1839'])
...
// console.log('dataSet:', dataSet, dates)
...
type D = keyof typeof exampleData

let earliestYear: number = 2000
let latestYear: number = 2000

Object.entries(dates).forEach(([key, data]) => {
  // console.log('key:', key)
  // const data = dates[key]
  // console.log('data:', data)
  const vuosi = Number(key.split('-')[0])
  if (vuosi < earliestYear) earliestYear = vuosi
    if (vuosi > latestYear) latestYear = vuosi
      dataSet.push({
        year: vuosi,
        startDate: parseTime(data['Jäätyminen'].Arvo),
        endDate: parseTime(data['Jäänlähtö'].Arvo),
